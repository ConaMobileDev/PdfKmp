...
- `:sample-web` — Kotlin/Wasm browser sample app (`wasmJs { browser() }` executable, no Compose). A static `index.html` + a button grid built from Kotlin over the no-asset `Samples.*` documents and a `pdfkmp-markdown` demo; clicks generate the PDF in-browser via the pure-Kotlin writer and hand it to the browser's viewer (`openInNewTab()`, download fallback). Run with `./gradlew :sample-web:wasmJsBrowserDevelopmentRun`. Depends on `:pdfkmp` + `:pdfkmp-markdown`. Not published.

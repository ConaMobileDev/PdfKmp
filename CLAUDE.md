...
- `:sample-desktop` — Compose for Desktop (JVM) sample app (`jvm()` target + `compose.desktop.application`). A window with sample-picker chips that renders each `Samples.*` document in `KmpPdfViewer`. Run with `./gradlew :sample-desktop:run`. Depends on all three publishable modules; proves the Desktop backend + viewer end-to-end.
